...
/******************************************************************//**
Put last pages from LRU to migration queue and flush it*/
ulint buf_LRU_to_migration_queue(){
//	ulint len;
//	ulint s;
//	ulint count = 0;
//	buf_pool_t* buf_pool;
...
//	migration_queue_t* m;
//	
//#ifdef UNIV_SBP_MQ_DEBUG
//	ullint start = ut_time_us(NULL);
//#endif
//
//	/* clear migration queue in all sbp instance */
//	for(s=0;s<srv_sbp_instances;s++){
//		buf_sec_pool=buf_sec_pool_ptr[s];
//
//		//mutex_enter(&buf_sec_pool->mutex);
//		/* we don't need mutex, because migration_queue is always single thread */
//		m = UT_LIST_GET_FIRST(buf_sec_pool->migration_queue);
//		/* clear the migration queue */
//		while(m){
//			migration_queue_t* q;
//			q = m;
//			m = UT_LIST_GET_NEXT(list,m);
//			UT_LIST_REMOVE(list,buf_sec_pool->migration_queue,q);
//			ut_free(q);
//		}
//
//		//mutex_exit(&buf_sec_pool->mutex);
//		ut_ad(UT_LIST_GET_LEN(buf_sec_pool->migration_queue) == 0 );
//	}
//
//	/* migration to queue */
//	for(s=0;s<srv_buf_pool_instances;s++){
//		buf_pool = &buf_pool_ptr[s];
//		buf_pool_mutex_enter(buf_pool);
//		
//		len = 1.0 * srv_sec_buf_pool_mq_pct  * UT_LIST_GET_LEN(buf_pool->LRU) / 100;
...
//
//		for( s=0; s < len; s++ ){
//			migration_queue_t* q = (migration_queue_t*)ut_malloc(sizeof(migration_queue_t));
//			ut_ad(bpage);
//			q->space = bpage->space;
//			q->offset = bpage->offset;
//			q->lsn = bpage->newest_modification;
//			buf_sec_pool = buf_sec_pool_get(q->space,q->offset);
//			//mutex_enter(&buf_sec_pool->mutex);
//			UT_LIST_ADD_FIRST(list,buf_sec_pool->migration_queue,q);
//			bpage = bpage->LRU.prev;
//			++count;
//			//mutex_exit(&buf_sec_pool->mutex);
//		}
//		buf_pool_mutex_exit(buf_pool);
...
//	}
//#ifdef UNIV_SBP_MQ_DEBUG
//	fprintf(stderr," InnoDB [Debug] SBP:To migration queue cost %.6f sec\n",( 1.0*ut_time_us(NULL) - start ) / 1000000 );
//	fflush(stderr);
//#endif
//
//	/* flush migration queue */
//	for(s=0;s<srv_sbp_instances;s++){
//		buf_sec_pool=buf_sec_pool_ptr[s];
//		mutex_enter(&buf_sec_pool->mutex);
//		m = UT_LIST_GET_FIRST(buf_sec_pool->migration_queue);
...
//		while(m){
//			buf_sec_block_t* blk;
//			buf_pool = buf_pool_get(m->space,m->offset);
//			buf_pool_mutex_enter(buf_pool);
//			mutex_enter(&buf_sec_pool->mutex);
//			HASH_SEARCH(hash,buf_sec_pool->page_hash,
//				buf_page_address_fold(m->space,m->offset),
//				buf_sec_block_t*,blk,
//				ut_ad(1),
//				m->space == blk->space && m->offset == blk->offset);
//			/* not in the seconadary buffer pool */
//			if ( blk == NULL ){
//				bpage = buf_page_hash_get(buf_pool,m->space, m->offset);
//				if ( bpage ){
//					mutex_enter(buf_page_get_mutex(bpage));
//					/* find the primary buffer pool */
//					if ( bpage->newest_modification == m->lsn ){
//						buf_sec_block_t* k;
//						/* not in the flush list */
...
//							k = UT_LIST_GET_LAST(buf_sec_pool->LRU);
//							/* if it is in the flush list, remove it first */
//							while( k->frame != NULL )
//								k = UT_LIST_GET_PREV(LRU,k);
//							UT_LIST_REMOVE(LRU,buf_sec_pool->LRU,k);
//							UT_LIST_ADD_LAST(free,buf_sec_pool->free,k);
//							/* remove from current hash table */
//							HASH_DELETE(buf_sec_block_t,hash,buf_sec_pool->page_hash,
//									buf_page_address_fold(k->space, k->offset),
//									k);
//						}
//						// Init block info
//						k = UT_LIST_GET_LAST(buf_sec_pool->free);
//						k->space = m->space;
//						k->offset = m->offset;
//						k->hash = NULL;
//						k->frame = NULL;
//						ut_a(os_file_write(srv_sec_buf_pool_file,buf_sec_pool->handle,((buf_block_t*)bpage)->frame,k->file_offset,k->file_offset_high,UNIV_PAGE_SIZE));
//						buf_sec_pool->stat.n_page_mq_flush++;
//
//						/* insert into secondary buffer pool hash table */
//						HASH_INSERT(buf_sec_block_t,hash,buf_sec_pool->page_hash,
//									buf_page_address_fold(k->space, k->offset),
//									k);
//						UT_LIST_REMOVE(free,buf_sec_pool->free,k);
//						UT_LIST_ADD_FIRST(LRU,buf_sec_pool->LRU,k);
//					}
...
alloc:
	...
	if ( srv_sec_buf_pool_size > 0 && bpage->space != 0 ){
		if ( buf_page_get_state(bpage) == BUF_BLOCK_FILE_PAGE 
			&& (fil_page_get_type(((buf_block_t*)bpage)->frame) == FIL_PAGE_INDEX || fil_page_get_type(((buf_block_t*)bpage)->frame) == FIL_PAGE_INODE  )
			){
			/* Read the first page of the tablespace if the size big enough */
			mybuf2 = ut_malloc(2 * UNIV_PAGE_SIZE);
			/* Align the memory for file i/o if we might have O_DIRECT set */
			mypage = ut_align(mybuf2, UNIV_PAGE_SIZE);
			ut_memcpy(mypage,((buf_block_t*)bpage)->frame,UNIV_PAGE_SIZE);
			space = bpage->space;
			offset = bpage->offset;
		}
	}
	...
	if ( srv_sec_buf_pool_size > 0 && mybuf2 != NULL )
	{
		buf_sec_pool_t* buf_sec_pool;
...
find:
		mutex_enter(&buf_sec_pool->mutex);
		if ( UT_LIST_GET_LEN(buf_sec_pool->bfree) == 0 ){
			...
			if ( block == NULL ){
				if ( UT_LIST_GET_LEN(buf_sec_pool->free) == 0 ){
					buf_sec_pool->stat.n_page_full_swap++;
					/* if free list is full, we should remove the last block in LRU */
					block = UT_LIST_GET_LAST(buf_sec_pool->LRU);
					while(block != NULL){
						mutex_enter(&block->mutex);
						/* in LRU list, bw must be NULL */
						ut_a(block->bw == NULL);
						/* if page is in read mode, try the previous page */
						if( block->io_fix != BUF_IO_NONE ){
							mutex_exit(&block->mutex);
					...
					/* if all the page is in the flush list, this should be almost impossible */
					ut_print_timestamp(stderr);
					fprintf(stderr," [Warning]: no space for secondary buffer pool LRU, waiting to flush.\n");
					mutex_exit(&block->mutex);
					mutex_exit(&buf_sec_pool->mutex);
					goto find;
finish:
				...
				/* insert into flush list */
				mutex_enter(&buf_sec_pool->flush_list_mutex);
				UT_LIST_ADD_LAST(flush_list,buf_sec_pool->flush_list,block);
				bw = UT_LIST_GET_LAST(buf_sec_pool->bfree);
				UT_LIST_REMOVE(free,buf_sec_pool->bfree,bw);
				block->bw = bw;
				...
				// init block info, do not any mutex on it
				ut_memcpy(bw->frame,mypage,UNIV_PAGE_SIZE);

				mutex_enter(&buf_sec_pool->mutex);
				mutex_enter(&block->mutex);
				block->io_fix = BUF_IO_NONE;
				UT_LIST_ADD_FIRST(used,buf_sec_pool->bused,block->bw);
				mutex_exit(&block->mutex);
				mutex_exit(&buf_sec_pool->mutex);
				...
				mutex_enter(&block->mutex);
				if ( block->bw )
					ut_ad(ut_memcmp(mypage,block->bw->frame,UNIV_PAGE_SIZE) ==0);
				else{
					byte*	buf3;
					byte*	page2;
					/* Read the first page of the tablespace if the size big enough */
					...
					/* Align the memory for file i/o if we might have O_DIRECT set */
					page2 = ut_align(buf3, UNIV_PAGE_SIZE);
					/* read from secondary buffer pool file */
					fil_io(OS_FILE_READ,TRUE,SBP_TB_ID+buf_sec_pool->instance_no,0,block->f_offset,0,UNIV_PAGE_SIZE,page2,NULL);
					//os_file_read(buf_sec_pool->handle,page2,block->file_offset,block->file_offset_high,UNIV_PAGE_SIZE);
					ut_ad(ut_memcmp(mypage,page2,UNIV_PAGE_SIZE) == 0 );
					ut_free(buf3);
				}
				mutex_exit(&block->mutex);
#endif
