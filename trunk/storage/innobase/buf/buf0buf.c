...
/** The secondary buffer pool of the database */
UNIV_INTERN buf_sec_pool_t*	buf_sec_pool_ptr[MAX_SEC_BUFFER_POOLS];
...
UNIV_INTERN mysql_pfs_key_t	buf_sec_pool_mutex_key[MAX_SEC_BUFFER_POOLS];
	...
	ulint	n_instances)	/*!< in: number of instances */
{
	...
	buf_pool_ptr = mem_zalloc((n_instances+srv_n_independent_bp) * sizeof *buf_pool_ptr);
	...
	if ( srv_n_independent_bp > 0 ){
		int j;
		srv_buf_pool_instances += srv_n_independent_bp;
		for( j=0; j<srv_n_independent_bp; j++ ){
			buf_pool_t*	ptr	= &buf_pool_ptr[j+i];
			if (buf_pool_init_instance(ptr, srv_independent_bp_sizes[j], j+i) != DB_SUCCESS) {
				/* Free all the instances created so far. */
				buf_pool_free(j+i);
				return(DB_ERROR);
			}
	...
	if ( srv_n_independent_bp > 0){
		...
		for(z=0;z<srv_n_independent_bp;z++){
			file = os_file_create_simple_no_error_handling(
				innodb_file_data_key, srv_independent_bp_names[z], OS_FILE_OPEN,
			...
			if (success){
				...
				srv_independent_bp_space[z] = fsp_header_get_space_id(page);
	...
	buf_page_t*	bpage)	/*!< in: pointer to the block in question */
{
	...
	if (io_type == BUF_IO_READ) {
		...
		if ( (bpage->space == TRX_SYS_SPACE
			|| (srv_doublewrite_file_size > 0 && bpage->space == DOUBLEWRITE_SPACE_ID) )
	...
	FILE*		file)		/*!< in/out: buffer where to print */
{
	...
	fprintf(file,
		...
		(ulong) buf_pool->instance_no + 1,
		(ulong) ( buf_pool->instance_no < srv_buf_pool_instances - srv_n_independent_bp ) ? "":srv_independent_bp_names[ buf_pool->instance_no - srv_buf_pool_instances + srv_n_independent_bp ], 
	...
	if ( buf_pool->instance_no >= 0 && buf_pool->instance_no < srv_buf_pool_instances-1 ){
		fprintf(file,"----------------------\n");
	}
	buf_pool_mutex_exit(buf_pool);
}
...
/*=======================*/
	...
	ulint instance_no,		/*!< in: instance number */
	ulint size,				/*!< in: each instance size */
	ulint offset)			/*!< in: offset in the secondary buffer pool file */
{
	ulint f;
	f = instance_no*size+offset;
	block->file_offset = f & 0xFFFFFFFFUL;
	block->file_offset_high = (ib_uint64_t)f >> 32;
	block->f_offset = f / UNIV_PAGE_SIZE;
	block->bw = NULL;
	block->io_fix = BUF_IO_NONE;
	mutex_create(PFS_NOT_INSTRUMENTED, &block->mutex, SYNC_BUF_BLOCK);
}
...
/*===============*/
{
	...
	for(i=0;i<srv_sbp_instances;i++)
		buf_sec_pool_free_instance(i);
}
/*********************************************************************//**
Prints info of the secondary buffer pool i/o. */
UNIV_INTERN
void
buf_sec_print_io_instances(
/*=========*/
	FILE*	file,	/*!< in/out: buffer where to print */
	ulint	instance_no) 
{
	...
	current_time = time(NULL);
	time_elapsed = 0.001 + difftime(current_time,
					buf_sec_pool->last_printout_time);

	fprintf(file,
		"Secondary buffer pool #%lu\n"
		"	size	%lu\n"
		"	Free pages	%lu\n"
		"	LRU pages	%lu\n"
		"	Flush list len %lu\n"
		"	State: %s\n",
		buf_sec_pool->instance_no+1,
	...
	fprintf(file,
		"	Page reads %lu, flush %lu, swap %lu, skip write overloaded %lu, skip unuseful %lu\n"
		"	Page reads %.2f/sec, flush %.2f/sec, swap %.2f/sec,migration flush %.2f/sec(in last %.2fsec)\n",
		...
		(buf_sec_pool->stat.n_page_reads - buf_sec_pool->old_stat.n_page_reads)/time_elapsed ,
		(buf_sec_pool->stat.n_page_flush - buf_sec_pool->old_stat.n_page_flush)/time_elapsed ,
		(buf_sec_pool->stat.n_page_swap - buf_sec_pool->old_stat.n_page_swap)/time_elapsed,
		(buf_sec_pool->stat.n_page_mq_flush - buf_sec_pool->old_stat.n_page_mq_flush)/time_elapsed,
		time_elapsed
		);

	buf_sec_pool->old_stat = buf_sec_pool->stat;
	buf_sec_pool->last_printout_time = ut_time();
...
) 
{
	...
	fprintf(file,"Secondary buffer pool hit ratio is: %.3f%%\n",100.0*srv_sec_buf_pool_reads / srv_buf_pool_reads);

	for(i=0;i<srv_sbp_instances;i++)
		buf_sec_print_io_instances(file,i);
	...
	ulint	n_instances)	/*!< in: number of instances */
{
	...
	for (i = 0; i < n_instances; i++) {
		...
		buf_sec_pool_ptr[i] = ptr;

		buf_sec_pool_init_instance(ptr,size,i);
...
UNIV_INTERN
void
buf_sec_pool_init_instance(buf_sec_pool_t* buf_sec_pool_ptr,ulint size, ulint instance_no)
{
	ulint i;
	...
	fil_space_create(srv_sec_buf_pool_file, SBP_TB_ID + instance_no, 0, FIL_SBP);
	fil_node_create(srv_sec_buf_pool_file, srv_sec_buf_pool_size, SBP_TB_ID + instance_no, FALSE);

	mutex_create(buf_sec_pool_mutex_key[instance_no],&buf_sec_pool_ptr->mutex,SYNC_BUF_SEC_POOL);
	mutex_create(buf_sec_pool_mutex_key[instance_no],&buf_sec_pool_ptr->flush_list_mutex,SYNC_BUF_SEC_POOL_FLUSH);
	
	mutex_enter(&buf_sec_pool_ptr->mutex);
	mutex_enter(&buf_sec_pool_ptr->flush_list_mutex);

	UT_LIST_INIT(buf_sec_pool_ptr->free);
	UT_LIST_INIT(buf_sec_pool_ptr->LRU);
	UT_LIST_INIT(buf_sec_pool_ptr->flush_list);
	UT_LIST_INIT(buf_sec_pool_ptr->migration_queue);
	UT_LIST_INIT(buf_sec_pool_ptr->bfree);
	UT_LIST_INIT(buf_sec_pool_ptr->bused);

	buf_sec_pool_ptr->size = size >> UNIV_PAGE_SIZE_SHIFT;
	/* init hash table for secondary buffer pool */
	buf_sec_pool_ptr->page_hash = hash_create(2 * buf_sec_pool_ptr->size);
	/* init buffer writes */
	buf_sec_pool_ptr->memory = (byte*)ut_malloc(UNIV_PAGE_SIZE*(srv_sec_buf_pool_buffered_writes+1));
	buf_sec_pool_ptr->frames = (byte*)ut_align(buf_sec_pool_ptr->memory,UNIV_PAGE_SIZE);
	buf_sec_pool_ptr->n_flush = 0;
	buf_sec_pool_ptr->no_flush = os_event_create(NULL);

	/* initialize buffered writes */
	for(i=0;i<srv_sec_buf_pool_buffered_writes;i++){
		bw = ut_malloc(sizeof(buffer_writes_t));
		bw->frame = &buf_sec_pool_ptr->frames[i*UNIV_PAGE_SIZE];
		UT_LIST_ADD_FIRST(free,buf_sec_pool_ptr->bfree,bw);
	}

	buf_sec_pool_ptr->instance_no = instance_no;
	buf_sec_pool_ptr->last_printout_time = ut_time();
	snprintf(buf_sec_pool_ptr->filename,strlen(srv_sec_buf_pool_file),"%s",srv_sec_buf_pool_file);

	for (i = 0; i <  buf_sec_pool_ptr->size; i++){
		block = ut_malloc(sizeof(buf_sec_block_t));
		buf_sec_block_init(block,instance_no,size,(ib_uint64_t)(i*UNIV_PAGE_SIZE));
		UT_LIST_ADD_FIRST(free,buf_sec_pool_ptr->free,block);
	}

	memset(&buf_sec_pool_ptr->stat,0x00,sizeof(buf_sec_pool_ptr->stat));
	memset(&buf_sec_pool_ptr->old_stat,0x00,sizeof(buf_sec_pool_ptr->stat));
	
	mutex_exit(&buf_sec_pool_ptr->flush_list_mutex);
	mutex_exit(&buf_sec_pool_ptr->mutex);
	
}

/********************************************************************//**
Completes an asynchronous read or write request of a file page to or from
the secondary buffer pool. */
UNIV_INTERN
void
buf_sec_block_io_complete(
/*=================*/
	buf_sec_block_t*	block)	/*!< in: pointer to the block in question */
{
	buf_sec_pool_t* sbp;
	buffer_writes_t* bw;

	ut_a(block->io_fix == BUF_IO_READ || block->io_fix == BUF_IO_WRITE);

	sbp = buf_sec_pool_get(block->space,block->offset);
	bw = block->bw;

	if ( block->io_fix == BUF_IO_READ ){
		mutex_enter(&sbp->mutex);
		mutex_enter(&block->mutex);
		ut_a(block->bw == NULL);
		block->io_fix = BUF_IO_NONE;
		if ( srv_sec_buf_pool_enable_lru ){
			/* add block to LRU FIRST */
			UT_LIST_REMOVE(LRU,sbp->LRU,block);
			UT_LIST_ADD_FIRST(LRU,sbp->LRU,block);
			sbp->stat.n_page_made_young++;
		}
		mutex_exit(&block->mutex);
		mutex_exit(&sbp->mutex);
	} else{
		buffer_writes_t* b;

		ut_a(block->io_fix == BUF_IO_WRITE);
		mutex_enter(&sbp->mutex);
		mutex_enter(&block->mutex);
		...
		UT_LIST_ADD_FIRST(LRU,sbp->LRU,block);
		/* move to free list */
		UT_LIST_REMOVE(used,sbp->bused,b);
		UT_LIST_ADD_FIRST(free,sbp->bfree,b);

		mutex_enter(&sbp->flush_list_mutex);
		/* remove from flush list */
		UT_LIST_REMOVE(flush_list,sbp->flush_list,block);
		mutex_exit(&sbp->flush_list_mutex);

		sbp->n_flush--;
		if (sbp->n_flush == 0) {
			/* The running flush batch has ended */
			//os_thread_sleep(20000000);
			os_event_set(sbp->no_flush);
		}
