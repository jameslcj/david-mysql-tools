...
#define BUF_POOL_WATCH_SIZE 1		/*!< Maximum number of concurrent
					buffer pool watches */
#define MAX_SEC_BUFFER_POOLS 16		/*!< The maximum number of secondary buffer
 					pools that can be defined */
...
extern ibool		buf_debug_prints;/*!< If this is set TRUE, the program
					...
					occurs */
#endif /* UNIV_DEBUG */
extern	buf_sec_pool_t* buf_sec_pool_ptr[MAX_SEC_BUFFER_POOLS];/*!< The secondary buffer pools
 					of the database */
...
struct buf_sec_block_struct{
	...
	//unsigned access_time:32;	/*!< first access time, zero if never access */
	...
	ulint file_offset; /*!< low offset */
	ulint file_offset_high; /*< high offset */
	ulint f_offset; /*< file offset in sbp file */
	buffer_writes_t*	bw;/* not NULL if it is in the buffered writes */
	mutex_t	mutex;/* to reduce bus traffic on sec pool mutex */
	//rw_lock_t	lock; /* protect frame */
	unsigned	io_fix:2;
};
...
struct buf_sec_pool_stat_struct{
	...
	ulint	n_dw_flush;
					/*!< number flush in double write */
	ulint	n_page_swap;
					/*!< number swap operations */
	ulint	n_page_full_swap;
					/*!< number swap operations on full LRU list */
	...
	ulint	n_page_skip_write_overloaded;
					/*!< number skip to secondary buffer pool
					because busy write */
	ulint	n_page_mq_flush;
};

struct migration_queue_struct{
	unsigned space:32;
	unsigned offset:32;
	ib_uint64_t lsn;
	UT_LIST_NODE_T(migration_queue_t) list;	/*!< hash chain */
};

struct buffer_writes_struct{
	byte*			frame; /* aligned buffered writes */
	UT_LIST_NODE_T(buffer_writes_t) free;	/*< free list */
	UT_LIST_NODE_T(buffer_writes_t) used; /*< flush_list */
};
...
struct buf_sec_pool_struct{
	...
	UT_LIST_BASE_NODE_T(migration_queue_t)	migration_queue;
					/*!< hash table of migration queue */
	ulint			size;
					/*!< pool size */
	mutex_t			mutex;
	...
	/** buffered writes */
	byte*			memory; /* buffered writes */
	byte*			frames; /* aligned buffered writes */
	mutex_t			bw_mutex; /* mutex protect bfree, bused */
	UT_LIST_BASE_NODE_T(buffer_writes_t) bfree;
	UT_LIST_BASE_NODE_T(buffer_writes_t) bused;
	/** flush variables */
	mutex_t			flush_list_mutex; /*!< protect Flush List */
	os_event_t		no_flush;
	ulint			n_flush;
};
