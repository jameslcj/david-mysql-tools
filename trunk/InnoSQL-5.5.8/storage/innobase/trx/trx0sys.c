...
start_again:
	...
	if (mach_read_from_4(doublewrite + TRX_SYS_DOUBLEWRITE_MAGIC)
		...
		fprintf(stderr, "InnoDB: Doublewrite buffer(TRX_SYS_SPACE) created\n");
    ...
    if (srv_doublewrite_file) {
		/* the same doublewrite buffer to DOUBLEWRITE_SPACE_ID should exist.
		check and create if not exist.*/

		mtr_start(&mtr);
		trx_doublewrite_buf_is_being_created = TRUE;

		block = buf_page_get(DOUBLEWRITE_SPACE_ID, 0, TRX_SYS_PAGE_NO,
					 RW_X_LATCH, &mtr);
		buf_block_dbg_add_level(block, SYNC_NO_ORDER_CHECK);
		...
		if (mach_read_from_4(doublewrite + TRX_SYS_DOUBLEWRITE_MAGIC)
			...
			block2 = fseg_create(DOUBLEWRITE_SPACE_ID, TRX_SYS_PAGE_NO,
						 TRX_SYS_DOUBLEWRITE
						 + TRX_SYS_DOUBLEWRITE_FSEG, &mtr);
			...
			new_block =
#endif /* UNIV_SYNC_DEBUG */
				buf_page_get(DOUBLEWRITE_SPACE_ID, 0, page_no,
							 RW_X_LATCH, &mtr);
				buf_block_dbg_add_level(new_block,
							SYNC_NO_ORDER_CHECK);
	...
	ibool	restore_corrupt_pages)	/*!< in: TRUE=restore pages */
{
	...
	dw_space_id = ( srv_doublewrite_file_size > 0 ) ? DOUBLEWRITE_SPACE_ID : TRX_SYS_SPACE;
	/* We do the file i/o past the buffer pool */

	if ( srv_doublewrite_file_size > 0 ){
		fprintf(stderr,"InnoDB: doublewrite file %s is used.\n",srv_doublewrite_file);
	}
	...
	fil_io(OS_FILE_READ, TRUE, dw_space_id, 0, TRX_SYS_PAGE_NO, 0,
	...
	fil_io(OS_FILE_READ, TRUE, dw_space_id, 0, block1, 0,
	       TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * UNIV_PAGE_SIZE,
	       buf, NULL);
	fil_io(OS_FILE_READ, TRUE, dw_space_id, 0, block2, 0,
	       TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * UNIV_PAGE_SIZE,
	...
	for (i = 0; i < TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * 2; i++) {
		...
		} else if ( (space_id == TRX_SYS_SPACE || (space_id==DOUBLEWRITE_SPACE_ID && srv_doublewrite_file_size>0))
			   && ((page_no >= block1
				&& page_no
